module digital_vault_addr::vault_v9 {
    ...
    public entry fun request_unlock(
        ...
        event::emit_event(&mut uc.unlock_events, UnlockEvent { capsule_id, unlocker: requester });
    }
    ...
    public fun get_capsule_info(
        owner_addr: address,
        capsule_id: u64
    ): (u8, address, u64, bool, bool) acquires UserCapsules {
        let uc = borrow_global<UserCapsules>(owner_addr);
        assert!(table::contains(&uc.capsules, capsule_id), error::not_found(E_CAPSULE_NOT_FOUND));
        let cap = table::borrow(&uc.capsules, capsule_id);
        (cap.capsule_type, cap.owner, cap.unlock_time, cap.is_unlocked, cap.is_geo_verified)
    }
    ...
    #[view]
    public fun get_main_file_bytes(
        requester: address,
        owner_addr: address,
        capsule_id: u64
    ): vector<u8> acquires UserCapsules {
